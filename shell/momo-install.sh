...
ls "$SCRIPT_DIR"/*.ipk >/dev/null 2>&1 || fail "未找到 Momo 安装包。"
...
log "正在安装本地 ipk 包..."
opkg install "$SCRIPT_DIR"/*.ipk || fail "Momo 安装失败，请检查系统版本是否为 OpenWrt 24.10+，以及依赖源是否完整。"

if ! opkg list-installed | grep -q '^luci-app-momo '; then
    fail "未检测到 luci-app-momo 已安装。"
fi
...
if [ -f /usr/share/luci/menu.d/luci-app-momo.json ] || [ -f /usr/lib/lua/luci/controller/momo.lua ]; then
    log "已检测到 LuCI 入口文件。"
else
    echo "警告：未检测到传统 LuCI 入口文件，请手动刷新页面并检查"服务"菜单。"
fi

log "安装完成，请在 LuCI 中打开 Momo 继续配置。"
